...
def frameAP(opt, print_info=True):
    ...
    for ilabel, label in enumerate(dataset.labels): # e.g.: 0, 'brush_hair
        ...
        # debug: compute individual class frame mAP
        ap_per_cls = 100 * np.array(pr_to_ap(results[label]))
    
    # orig: evaluating all classes and stack into a variable
...
if __name__ == "__main__":
    ...
    if opt.task == 'BuildTubes':
        if opt.rgb_model is not '':
            raise NotImplementedError('Currently BuildTubes mode only supports micro-motion (mm) models. One can easily adapt the code for rgb model.')
        BuildTubes(opt)
    elif opt.task == 'frameAP':
        frameAP(opt)
    elif opt.task == 'videoAP':
        if opt.rgb_model is not '':
            raise NotImplementedError('Currently videoAP only supports micro-motion (mm) models. One can easily adapt the code for rgb model.')
        videoAP(opt)
